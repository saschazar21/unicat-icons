...
const fileList = ["../README.md", "../CHANGELOG.md", "../LICENSE"];
const linkList = [];
...
const copier = async () => {
  ...
  await Promise.all(fileList.map(async f => {
    try {
      const fileName = basename(resolve(__dirname, f), '.md');
      await copy(resolve(__dirname, f), resolve(pageRoot, `${fileName}.md`))
        .then(
          () => console.log(`${resolve(pageRoot, f)} successfully copied!`),
        )
        .then(() => linkList.push(basename(fileName.toLowerCase())));
