...
const { copy, emptyDir, ensureDir, writeFile } = require("fs-extra");
...
const copier = async () => {
  ...
  fileList.forEach(async f => {
    try {
      ...
      await copy(resolve(__dirname, f), resolve(pageRoot, fileName)).then(
        () => `${resolve(pageRoot, f)} successfully copied!`
      );
    } catch (e) {
      console.log(
        `${resolve(
