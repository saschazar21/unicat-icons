...
const writeIcon = async ({ name, contents }) => ensureDir(resolve(buildDir, process.env.TARGET))
  .then(() => svgr(contents, svgrConfig, { componentName: `${pascalCase(name)}Icon` }))
  .then(jsx => writeFile(`${resolve(buildDir, process.env.TARGET, `${paramCase(name)}.tsx`)}`, jsx));

module.exports = async () => {
  const icons = await read();
  const iconPaths = icons.map(({ name }) => `export { default as ${pascalCase(name)}Icon } from './${process.env.TARGET}/${paramCase(name)}'`);
